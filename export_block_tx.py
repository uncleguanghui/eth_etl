...
def export(web3, start, end, batch, output, continue_=False, waiting=False):
    for start_block in range(start, end, batch):
        ...
        # 递归创建 blocks 目录
        dir_blocks = os.path.join(output, 'blocks', f'start={start_block:08d}/end={end_block:08d}')
        ...
        dir_txs = os.path.join(output, 'transactions', f'start={start_block:08d}/end={end_block:08d}')
